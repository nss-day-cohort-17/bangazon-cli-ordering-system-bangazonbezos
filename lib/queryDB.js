...
module.exports.getPopularity = ()=>{
  return new Promise ((resolve,reject)=>{
    db.all(`SELECT products.name,products.price*order_line_items.quanity AS revenue ,order_line_items.quanity FROM products
            JOIN order_line_items ON order_line_items.productid = products.productid
            ORDER BY order_line_items.quanity DESC`,(err,data)=>{
              if(err) return(console.log(err));
