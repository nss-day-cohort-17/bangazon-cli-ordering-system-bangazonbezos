...
module.exports.getActiveCustomers = ()=>{
    ...
    return new Promise((resolve,reject)=>{
      ...
      db.all(`SELECT order_line_items.lineitemid,order_line_items.productid,products.name AS productname,order_line_items.quanity FROM order_line_items
              JOIN products ON products.productid = order_line_items.productid WHERE order_line_items.orderid = ${currentObj.orderid}`,(err,data)=>{
                if(err) return(console.log('no items are in your cart'))
                  let orderLineIdArr = [];
                for (let i in data) {
  ...
  module.exports.getUnpaidCustomer = ()=>{
    return new Promise((resolve,reject)=>{
      db.all(`SELECT customers.firstname||" "||customers.lastname AS CustomerName,(products.price*order_line_items.quanity) AS orderTotal FROM order_line_items
               JOIN orders ON orders.orderid = order_line_items.orderid
               JOIN customers ON customers.customerid = orders.customerid
               JOIN products ON products.productid = order_line_items.productid
               WHERE orders.paidinfull = 0
              `,(err,data)=>{
        if(err) return(console.log(err));
        resolve(data)
      });
